...
export const loadMineStage = (w: number, h: number, stats: PlayerStats, addObject: (obj: LevelObject) => void) => {
    ...
    // Static Boundaries (Crimson Bedrock IDs)
    ...
    // 0 = Empty, 1 = Wall, 2 = Permanent Wall (Bedrock/Border)
    const grid: number[][] = Array(cols).fill(0).map(() => Array(rows).fill(1));

    // Initialize Grid with Noise
    for(let x=0; x<cols; x++) { 
        for(let y=0; y<rows; y++) { 
            if (x === cols-1) grid[x][y] = 2; 
            else if (Math.random() < 0.38) grid[x][y] = 2; // Initial random fill
        } 
    }
            ...
            for(let y=1; y<rows-1; y++) { 
                ...
                for(let dx=-1; dx<=1; dx++) { 
                    for(let dy=-1; dy<=1; dy++) { 
                        if (grid[x+dx][y+dy] === 2) neighbors++; 
                    } 
                } 
                if (neighbors >= 5) nextGrid[x][y] = 2; 
                else nextGrid[x][y] = 1; 
            } 
        } 
        for(let x=0; x<cols; x++) { 
    ...
    // Perlin-like Noise for large open caves
    ...
    const stretchY = 0.5; 
    const noiseW = Math.ceil(cols * noiseScale) + 1; 
    const noiseH = Math.ceil(rows * noiseScale * stretchY) + 1; 
    const noiseLattice = new Float32Array(noiseW * noiseH).map(() => Math.random());
    
    const getNoise = (x: number, y: number) => { 
        const nx = x * noiseScale; 
        const ny = y * noiseScale * stretchY; 
        ...
        const sx = nx - x0; 
        const sy = ny - y0; 
        const n00 = noiseLattice[y0 * noiseW + x0] || 0; 
        const n10 = noiseLattice[y0 * noiseW + x1] || 0; 
        const n01 = noiseLattice[y1 * noiseW + x0] || 0; 
        const n11 = noiseLattice[y1 * noiseW + x1] || 0; 
        const ix0 = cosineInterpolate(n00, n10, sx); 
        const ix1 = cosineInterpolate(n01, n11, sx); 
        return cosineInterpolate(ix0, ix1, sy); 
    };

    // Apply Noise to carve out caves
    for(let x=0; x<cols; x++) { 
        for(let y=0; y<rows; y++) { 
            const n = getNoise(x, y); 
            if (n > 0.6) { 
                if (grid[x][y] === 1) { grid[x][y] = 0; } 
                else if (grid[x][y] === 2 && n > 0.8) { grid[x][y] = 0; } 
            } 
        } 
    }
    
    // Ensure Start Area and Bottom are clean
    for(let x=0; x<15; x++) { 
        for(let y=0; y<rows; y++) { 
            if (grid[x][y] === 2) grid[x][y] = 1; 
            if (y > rows - 4) grid[x][y] = 1; 
        } 
    }

    // --- ORE GENERATION ---
    const scannerLevel = 1; // Default level as upgrade was removed
    const spawnVein = (centerX: number, centerY: number, typeCode: number, size: number) => { 
        for(let i=0; i<size; i++) { 
            const ox = centerX + Math.floor((Math.random() - 0.5) * 4); 
            const oy = centerY + Math.floor((Math.random() - 0.5) * 4); 
            if (ox >= 0 && ox < cols && oy >= 0 && oy < rows) { 
                if (grid[ox][oy] === 1) { grid[ox][oy] = typeCode; } 
            } 
        } 
    };
    ...
    // --- CONVERT GRID TO OBJECTS (Optimized Merging) ---
    ...
    for (let y = 0; y < rows; y++) { 
        for (let x = 0; x < cols; x++) { 
            ...
            // Merge horizontal walls for performance
            if (cellType === 2) { 
                let width = 1; 
                ...
                // ID prefix "wall_" helps renderer identify it as crimson wall
                addObject({ x: worldX, y: worldY, width: width * BLOCK_SIZE, height: BLOCK_SIZE, type: 'solid', id: `wall_${x}_${y}` }); 
                x += width - 1; 
                continue; 
            } 
            
            // Merge destructible blocks into 4x4 or 2x2 chunks if possible
            let size = 1; 
            ...
            let health = 250 * (size * size * 0.5); // Was 80, now 250 (3x tougher)
    
            let variant = 0;

            switch (cellType) { 
                case 3: type = 'scrap'; health = 120; break; 
                case 4: type = 'ice'; health = 250; break; 
                case 5: type = 'coal'; health = 150; break; 
                case 6: type = 'wood'; health = 180; break; 
                case 7: type = 'iron'; health = 350; break; 
                case 8: type = 'titanium'; health = 800; break; 
                case 9: type = 'uranium'; health = 500; break; 
                default: 
                    type = 'living_metal'; 
                    health = 200;
                    if (Math.random() < 0.13) { 
                    ...
                    else if (Math.random() < 0.05) { variant = 1; health = 300; }
                break; 
            } 
            if (size > 1) health *= 2; 
